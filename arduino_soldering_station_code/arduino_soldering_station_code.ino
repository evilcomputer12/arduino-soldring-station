...
void setup(){
  ...
  lcd.begin();
  lcd.clear();
  lcd.setCursor(0,1);lcd.print("Nagodena T: ");  
  lcd.setCursor(0,0);lcd.print("Izmerena T: "); 
}
...
void loop(){
  ...
  if (currentMillis - previousMillis >= lcdInterval){ //LCD will only display new data ever n milisec intervals
    ...
    if(analogRead(knob)==0){
      lcd.setCursor(10,1);lcd.print(" ISK  ");
      Serial.println(knob);
    }
    else{
    ...
    if(currentTemp<minTemp){
      lcd.setCursor(10,0);lcd.print("AMB ");
    }
    else{
